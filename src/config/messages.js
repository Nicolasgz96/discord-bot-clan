...
const MESSAGES = {
  ...
  // Welcome Messages
  WELCOME: {
    NEW_MEMBER: (memberName) => `${EMOJIS.CASTLE} Un nuevo cazador de demonios entra al dojo. ¡Bienvenido, **${memberName}**!`,
  ...
  // Playlists
  PLAYLISTS: {
    CREATED: (name) => `${EMOJIS.SUCCESS} Playlist **${name}** creada exitosamente. Usa \`/playlist agregar\` para añadir canciones.`,
    SAVED: (name, count) => `${EMOJIS.SUCCESS} **Playlist guardada exitosamente**\n📚 **${name}**\n🎵 ${count} canción${count !== 1 ? 'es' : ''} guardada${count !== 1 ? 's' : ''}`,
    LOADED: (name, count) => `${EMOJIS.PLAY} Playlist **${name}** cargada con ${count} canción${count !== 1 ? 'es' : ''}.`,
    NOT_FOUND: (name) => `${EMOJIS.ERROR} No se encontró la playlist **${name}**.`,
    EMPTY: (name) => `${EMOJIS.INFO} La playlist **${name}** está vacía.`,
    ALREADY_EXISTS: (name) => `${EMOJIS.WARNING} Ya tienes una playlist llamada **${name}**. Usa otro nombre.`,
    DELETED: (name) => `${EMOJIS.SUCCESS} Playlist **${name}** eliminada permanentemente.`,
    RENAMED: (oldName, newName) => `${EMOJIS.SUCCESS} Playlist **${oldName}** renombrada a **${newName}**.`,
    SONG_ADDED: (songTitle, playlistName) => `${EMOJIS.SUCCESS} **${songTitle}** agregada a la playlist **${playlistName}**.`,
    SONG_REMOVED: (songTitle, playlistName) => `${EMOJIS.SUCCESS} **${songTitle}** eliminada de la playlist **${playlistName}**.`,
    LIMIT_REACHED: (limit) => `${EMOJIS.WARNING} Has alcanzado el límite de ${limit} playlists. Elimina alguna para crear una nueva.`,
    SONG_LIMIT_REACHED: (limit) => `${EMOJIS.WARNING} Esta playlist alcanzó el límite de ${limit} canciones.`,
    NAME_TOO_SHORT: `${EMOJIS.WARNING} El nombre de la playlist debe tener entre 2 y 50 caracteres.`,
    NO_PLAYLISTS: `${EMOJIS.INFO} No tienes playlists guardadas. Crea una con \`/playlist crear\`.`,
    QUEUE_EMPTY: `${EMOJIS.WARNING} No hay canciones en la cola para guardar.`,
  },

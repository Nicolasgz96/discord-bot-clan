...
setInterval(() => {
  ...
  for (const [userId, data] of usernameCache.entries()) {
    // Si la entrada del cache tiene más de 1 hora, eliminarla
...
client.once(Events.ClientReady, async (c) => {
  ...
  console.log(`${EMOJIS.SUCCESS} Bot en línea como ${c.user.tag}`);
...
// Se usa para desconectar el bot automáticamente si se queda solo
...
// Función auxiliar para enviar mensajes con lógica de reintento
...
// Función auxiliar para obtener el emoji de un rango
...
// ==================== MANEJADOR DE BOTONES INTERACTIVOS DE MÚSICA (TIPO HYDRA) ====================
client.on(Events.InteractionCreate, async (interaction) => {
  ...
  if (interaction.customId.startsWith('playlist_load_')) {
    ...
    return; // Salir después de manejar el botón de playlist
  }
...
// ==================== MANEJADOR DE MODALES (GUARDAR PLAYLIST) ====================
client.on(Events.InteractionCreate, async (interaction) => {
